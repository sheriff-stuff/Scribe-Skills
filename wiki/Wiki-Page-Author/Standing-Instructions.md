# Standing Instructions

- New pages use the [page template](../../plugins/wiki-page-author/skills/wiki-page-author/assets/page-template).
- New `> [!ODD]` blocks use the [Open Design Decision template](../../plugins/wiki-page-author/skills/wiki-page-author/assets/odd-template).
- New `> [!CAUTION]` blocks use the [Page Investigation Caution template](../../plugins/wiki-page-author/skills/wiki-page-author/assets/caution-template).
- [`home.md`](../home) stays in sync — when a page is added or removed, [`home.md`](../home) is updated in the same operation.
- When a pointer ODD block is added to another page, the owner page's `Affects:` line is updated in the same operation.
- Inconsistencies introduced by a change are flagged to the user. Other pages are not edited unprompted.
