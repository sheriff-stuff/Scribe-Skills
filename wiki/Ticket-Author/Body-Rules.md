...
- Every statement earns its place in the section it appears in. A statement that adds nothing beyond what another section already covers is removed. Elaboration, specificity, and implementation detail are not repetition — touching the same topic is fine when something new is added.
...
Reference formatting — absolute URLs, and plain-text titles for sibling tickets created in the same branch — follows the consuming project's `CLAUDE.md`.
