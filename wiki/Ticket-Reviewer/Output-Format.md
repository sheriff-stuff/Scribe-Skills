...
The numeric line is required and matches that exact format. `N` is the count of `VERDICT: READY` files and `M` the total reviewed, both derived from the verdicts above. The `NOTES:` line is emitted when:
